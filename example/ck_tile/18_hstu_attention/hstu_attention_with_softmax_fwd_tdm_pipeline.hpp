...
// TDM (Tensor Data Mover) variant of the with-softmax forward pipeline, targeting gfx1250.
//
...
//
// The softmax state (m/l) is carried in registers only: the row-wise reductions go through
// ds_bpermute (wave-internal lane crossing), so they add neither LDS nor a workgroup
// barrier on top of the deep ping-pong schedule inherited from the no-softmax variant.
...
struct HstuAttentionWithSoftmaxFwdPipelineQRKSVSTdm
{
    ...
    static_assert(kQKHeaddim == 128,
                  "hstu with-softmax tdm pipeline is only gated in for MaxK == 128; other tile "
               ...
               DropoutType& dropout) const
    {
        ...
        // m/l are indexed by the M-span of gemm_0's C tile here, but by the M-span of
        // gemm_1's C tile (o_acc) in the rescale/normalize sweeps below. Both gemms use
        // sequence<4,1,1> block warps, so the two M-spans pair up element-wise; a tile
        // setting that splits them differently would silently mis-scale o_acc.
        ...
        //
        // This is the only schedule this pipeline implements, so assert the preconditions
        // instead of silently falling back to an untested one.
        static_assert(n0_loops == 1 && NumKVLdsBuffers >= 4,
                      "hstu with-softmax tdm pipeline only implements the deep (ping-pong) "
                      "schedule, which needs a single n0 sub-loop and a spare half of the "
                      "K/V LDS buffers");

        auto k_lds_cur  = k_lds_windows[number<0>{}];
        auto k_lds_next = k_lds_windows[number<1 % NumKVLdsBuffers>{}];
        ...
        do
        {
            ...
            // Keep the softmax epilogue (the o_acc rescale above in particular) from being
            // interleaved with gemm_1's ds_read_tr: both sides use the LDS pipeline, the
            // softmax side through ds_bpermute, and the rescale must land before gemm_1
            // accumulates into o_acc. [inferred, unverified - needs an ISA dump]
            __builtin_amdgcn_sched_barrier(0x00000001);
