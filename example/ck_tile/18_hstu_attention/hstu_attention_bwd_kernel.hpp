...
struct HstuAttentionBwdDQDKDVSoftmaxKernel
{
    ...
    struct Kargs
    {
        ...
        index_t nhead_stride_dv;
        index_t nhead_stride_dq_acc;
        index_t nhead_stride_lsed; // LSE and D share this
    ...
    CK_TILE_DEVICE void operator()(Kargs kargs) const
    {
        ...
        long_index_t batch_offset_dv;
        long_index_t batch_offset_dq_acc;
        long_index_t batch_offset_lsed; // LSE and D share the same per-(batch) base
        ...
        if(kargs.is_jagged)
        {
            ...
            batch_offset_dv     = key_start * kargs.stride_dv;
            batch_offset_dq_acc = query_start * kargs.stride_dq_acc;
            batch_offset_lsed   = query_start; // LSE/D seq stride is 1 (packed)
        ...
        // LSE / D: 1-D packed over seqlen_q (seq stride 1), mirrors FMHA bwd kernel.
        const auto lse_dram = make_naive_tensor_view_packed<address_space_enum::global>(
            lse_ptr, make_tuple(kargs.seqlen_q), number<HstuPipeline::kM0>{});
        const auto d_dram = make_naive_tensor_view_packed<address_space_enum::global>(
            d_ptr, make_tuple(kargs.seqlen_q), number<HstuPipeline::kM0>{});
...
struct HstuAttentionBwdDQDKDVGroupSoftmaxKernel
{
    ...
    CK_TILE_DEVICE void operator()(Kargs kargs) const
    {
        ...
        const long_index_t batch_offset_dv     = key_start * kargs.stride_dv;
        const long_index_t batch_offset_dq_acc = query_start * kargs.stride_dq_acc;
        const long_index_t batch_offset_lsed   = query_start; // LSE/D seq stride 1 (packed)
        ...
        const auto lse_dram = make_naive_tensor_view_packed<address_space_enum::global>(
            lse_ptr, make_tuple(seqlen_q), number<P::kM0>{});
        const auto d_dram = make_naive_tensor_view_packed<address_space_enum::global>(
            d_ptr, make_tuple(seqlen_q), number<P::kM0>{});
...
// softmax MAIN reads. hdim_v=64 is small, so a single-thread accumulate is fine.
